...
class DDPG(object):
    ...
    st.markdown("""
        <div class="home-container">
            <h1>114(上)學年度『智慧機械設計』課程期末報告</h1>
            <h2>ROS自主移動平台與AI整合之研究分工表</h2>
            <h3>指導老師：周榮源</h3>
            <h3>班級：碩設計一甲</h3>
            <h3>組別：第一組</h3>
            <h3>組員：11473132 陳威誌、11473105  紀閔翔、11473143 朱王黃</h3>           
            <br>
        </div>
    """, unsafe_allow_html=True)
    st.markdown("<br>", unsafe_allow_html=True)
    ...
    df = pd.DataFrame({
        "學號": [11473105, 11473132, 11473143],
        "姓名": ["紀閔翔", "陳威誌", "朱王黃"],
        "分工內容": ["RL建模與訓練、TurtleBot、深度學習實作與報告討論", "全篇主要程式架構撰寫、除錯與網頁報告統整",
                     "YOLO的照片標註、程式討論、步驟截圖與報告構思"],
        "貢獻度": ["40%", "30%", "30%"]
    })
